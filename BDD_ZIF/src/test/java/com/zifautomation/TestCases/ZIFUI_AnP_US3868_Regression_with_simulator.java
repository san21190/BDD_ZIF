...
public class ZIFUI_AnP_US3868_Regression_with_simulator extends Base {
	...
	@Test
	public void ZIFUI_AnP_US3868_Regression_with_simulator() throws IOException, InterruptedException {

		//Report Initialization

		test = extent.createTest("ZIFUI_AnP_US3868_Regression_with_simulator");
		test.createNode("ZIFUI_AnP_US3868_Regression_with_simulator");
		...
		try{
			...
			Date date = new Date();
			System.out.println("Case count from case management screen with simulator " +case_count);
			System.out.println("Time when the count is taken "+formatter.format(date));
			test.log(Status.PASS, "Case management screen with simulator count and the time"+"  "+case_count+"  "+formatter.format(date));
			test.addScreenCaptureFromPath(captureScreenShot(driver));
			Thread.sleep(3000);
		}
		catch (AssertionError | Exception e) {
		...
		try{
			...
			SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
			Date date = new Date();
			System.out.println("Count in Alerts screen with simulator "+case_count);
			System.out.println("Time when the count is taken "+formatter.format(date));
			test.log(Status.PASS, "Alerts screen with simulator count and the time"+"  "+case_count+"  "+formatter.format(date));
			test.addScreenCaptureFromPath(captureScreenShot(driver));
			Thread.sleep(3000);
		}
		catch (AssertionError | Exception e) {
